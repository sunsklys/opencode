...
clean-state: ## 清理运行时状态文件（保留 plans/boulder.json/start-work）
	@echo "清理运行时状态文件..."
	@node -e "const fs=require('fs');const targets=['.omo/run-continuation','tasks'];let cleaned=0;for(const t of targets){if(fs.existsSync(t)){fs.rmSync(t,{recursive:true,force:true});console.log('  ✓ '+t+' 已清理');cleaned++}}if(cleaned===0)console.log('  无运行时残留')"
