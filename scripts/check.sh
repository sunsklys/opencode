...
echo "【3/12·Warning】opencode-mem 记忆插件"
...
# ---------- 6. [Warning] Web UI ----------
echo "【6/12·Warning】opencode-mem Web UI"
...
echo "【7/12·Warning】plugin @latest 漂移检测（opencode 缓存 vs 项目软链）"
...
echo "【8/12·Warning】skills SHA256 校验（供应链完整性，lark + OMO）"
...
echo "【9/12·Warning】oh-my-openagent 内置 skill 软链健康（含自愈）"
...
echo "【10/12·Critical】opencode plugin 缓存健康（dist/skills 完整性）"
...
if [ "$PROJECT_COUNT" -ne 20 ]; then
  ...
  if [ "$PLUGIN_COUNT" -eq 20 ]; then
    ...
    fail "plugin 缓存不完整（$PLUGIN_COUNT/20）— 运行 make update"
  fi
  ...
  fail "builtin 缓存不完整（$BUILTIN_COUNT/20）— 运行 make update"
else
  fail "plugin 缓存不完整（$PLUGIN_COUNT/20）— 运行 make update"
fi

# ---------- 11. [Critical] OMO + opencode 关键字段验证 ----------
echo "【11/12·Critical】OMO + opencode 关键字段配置验证"
...
echo "【12/12·Warning】tui.json plugin 字段与 opencode.json 同步"
